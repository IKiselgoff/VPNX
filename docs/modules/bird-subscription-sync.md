...
## Зависимости
`curl`, системный Python 3, `launchd`, CLI `vpnx`, HTTPS endpoint BIRD VPN и необязательный локальный bootstrap `~/.vpnx/bird-bootstrap.json`.
...
## Failure Modes
При отсутствии сети используется локальный bootstrap, если он установлен; без него существующий snapshot сохраняется. Некорректный полученный payload не заменяется fallback-файлом и останавливает применение. `launchd` повторит запуск через 15 минут. Неизвестная страна останавливает применение всего snapshot, чтобы не получить частичное обновление.
