...
Переносимая Android-сборка содержит актуальный bootstrap BIRD и URL подписки. При первом открытии она синхронизирует профили и показывает системные battery-optimization и VPN consent; после подтверждения выбранный Auto WiFi профиль запускается автоматически. Maintenance SSH credentials являются индивидуальными для устройства и в переносимую APK не включаются.
